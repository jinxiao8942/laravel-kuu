...
function changeTimeValue( val ){
	...
	if( hrs > 0 ){
		...
		if( mil_sec > 0 ){
			val = Math.round( (val + (mil_sec / 1000)) * 100 ) / 100;
		}
				
		val = val + " sec";
	} else if( mil_sec > 0 ){
		val = Math.round( (mil_sec / 1000) * 100 ) / 100 + " sec";
	}
...
function check_list_see_more(){
	jQuery(".graph-page-list-items a").click(function(){
		pagecount = parseInt( jQuery(this).parent().parent().attr("data-page-count") );
		pageindex = parseInt( jQuery(this).parent().attr("data-index") );
		...
		if( pagecount > 4 ){
			...
			if( pageindex == 0 ){
				...
				jQuery(".graph-page-prev").addClass( "hide" );
				jQuery(".graph-page-next").removeClass( "hide" );
			
			} else if( pageindex == 1 ){
				start_index = 1;
				end_index = 4;
				
				jQuery(".graph-page-prev").removeClass( "hide" );
				jQuery(".graph-page-next").removeClass( "hide" );
				
			} else if( pagecount - ( pageindex + 1 ) == 0 ) {
				start_index = pageindex - 3;
				end_index = pageindex;
				
				jQuery(".graph-page-prev").removeClass( "hide" );
				jQuery(".graph-page-next").addClass( "hide" );
				
			}  else if( pagecount - ( pageindex + 1 ) == 1 ) {
				start_index = pageindex - 2;
				end_index = pageindex + 1;
			...
			for( var i = 0; i < pagecount ; i++ ){
				if( start_index <= i && i <= end_index ){
					jQuery("#pagelist-" + i).removeClass("hide");
				} else {
					jQuery("#pagelist-" + i).addClass("hide");
				}
			}
		} else if( pagecount > 1 ) {
		...
		jQuery.ajax({
			...
			success : function( res ){
                if(res.hasOwnProperty('expired')) {
                    jQuery("#graph-section-bar").html(res.html);
...
function check_list_action_list(){	
	jQuery(".see_detail_btn").click(function(){
		...
		$detail_page_list = '<ul data-page-count="' + $detail_page_url.length + '">';
		for( var i = 0; i < $detail_page_url.length; i++ ){
			...
			$detail_page_list += '<li id="pagelist-' + i + '" ' + hide_class + ' data-index="' + i + '"><a href="" class="' + jQuery($detail_page_url[i]).attr('class') + '" checktype="' + jQuery($detail_page_url[i]).attr('checktype') + '" mongoid="' + jQuery($detail_page_url[i]).attr('mongoid') + '" checkid="' + jQuery($detail_page_url[i]).attr('checkid') + '" passday="' + jQuery($detail_page_url[i]).attr('passday') + '" is_paused="' + jQuery($detail_page_url[i]).attr('is_paused') + '">' + jQuery($detail_page_url[i]).html() + '</a></li>';
		}
		$detail_page_list += '</ul>';
...
jQuery(document).ready(function(){	
	...
	jQuery("#data-selection-list-suspend-btn").click(function(){
		if( confirm(confirm_text.suspendcheck) ){
			show_dashboard_graph_mask();
			...
			jQuery.ajax({
				...
				success : function( res ){
					...
					jQuery("#check_row_" + $checkid).addClass('passed-checked');
				}
			});
		}
	});
	...
	jQuery("#data-selection-list-unsuspend-btn").click(function(){
		if( confirm(confirm_text.unsuspendcheck) ){
			show_dashboard_graph_mask();
			...
			jQuery.ajax({
				...
				success : function( res ){
					...
					jQuery("#check_row_" + $checkid).removeClass('passed-checked');
				}
			});
		}
	});
	...
	jQuery("#data-selection-list-delete-btn").click(function(){
		if( confirm(confirm_text.delete_c) ){
			show_dashboard_graph_mask();
			...
			jQuery.ajax({
				...
				success : function( res ){				
					...
					if( res.is_add_enable ){
						jQuery(".add-site-section").removeClass("hide");
						jQuery("#add-limit-message-section").addClass("hide");
					} else {
						jQuery(".add-site-section").addClass("hide");
						jQuery("#add-limit-message-section").removeClass("hide");
					}
				}
			});
		}
			...
			success : function( res ){
				...
				if( res.type == 'http' ){
					...
					for( var i = 0; i < res.alert_email.length; i++ ){
						notification_email_html += '<li><span class="pull-left">' + res.alert_email[i]['alert_email'] + '</span><a class="remove_check_alert_email pull-right" href="#"><span class="small-close"></span></a><input type="hidden" name="check_alert_email[]" value="' + res.alert_email[i]['alert_email'] + '"></li>';
					}
	...
	jQuery("#edit_http_add_alert_email").click(function(){
		...
		if( pattern.test( alert_email ) ) {
			jQuery("#edit_http_alert_list").append('<li><span class="pull-left">' + alert_email + '</span><a class="remove_check_alert_email pull-right" href="#"><span class="small-close"></span></a><input type="hidden" name="check_alert_email[]" value="' + alert_email + '"></li>');
	...
	jQuery("#edit_https_add_alert_email").click(function(){
		...
		if( pattern.test( alert_email ) ) {
			jQuery("#edit_https_alert_list").append('<li><span class="pull-left">' + alert_email + '</span><a class="remove_check_alert_email pull-right" href="#"><span class="small-close"></span></a><input type="hidden" name="check_alert_email[]" value="' + alert_email + '"></li>');
	...
	jQuery("#edit_dns_add_alert_email").click(function(){
		...
		if( pattern.test( alert_email ) ) {
			jQuery("#edit_dns_alert_list").append('<li><span class="pull-left">' + alert_email + '</span><a class="remove_check_alert_email pull-right" href="#"><span class="small-close"></span></a><input type="hidden" name="check_alert_email[]" value="' + alert_email + '"></li>');
	...
	//When No Checks - use Add Site as default
	$domain_list_count = jQuery(".domain-section").length;
	if( $domain_list_count == 0 ){
		jQuery("#add_site_btn").trigger("click");
	}
});
