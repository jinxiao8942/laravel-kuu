...
class UserSiteInfo extends Eloquent{
	...
	function getUserCheckCount( $userid ){
		$query = "SELECT COUNT(check_id) as check_count FROM {$this->table} WHERE user_id={$userid}";
		
		$result = DB::select($query);
		
		if( isset( $result[0]->check_count ) ) return $result[0]->check_count;
	...
	function getUserSiteInfoByCheckId( $checkid ){
		$query = "SELECT check_id, mongo_id, type, domain, url, host, options, create_time, paused FROM {$this->table} WHERE check_id={$checkid}";
		...
		if( is_array( $dataList ) && count( $dataList ) > 0 ){
			...
			$site_info['create_time'] = $dataList[0]->create_time;
			$site_info['paused'] = $dataList[0]->paused;
		}
		
		return $site_info;
	}
	
	function getDetailUserSiteInfoByCheckId( $checkid ){
		$query = "SELECT check_id, mongo_id, type, domain, url, host, options, create_time, paused FROM {$this->table} WHERE check_id={$checkid}";
		$dataList = DB::select( $query );
		...
		if( is_array( $dataList ) && count( $dataList ) > 0 ){
			...
			if( ( $currentTime - $dataList[0]->create_time ) > 86400 ){
				$mongo_id_list_after_one_day[] = $dataList[0]->mongo_id;
			} else {
				$mongo_id_list_less_one_day[] = $dataList[0]->mongo_id;
			}
			...
			if( isset($lastDayCheckStates[$dataList[0]->mongo_id]) ){
				$site_info['uptime'] = round( $lastDayCheckStates[$dataList[0]->mongo_id]->availability * 100 ) / 100;
				$site_info['uptime'] .= "%";
				$site_info['response_speed'] = $lastDayCheckStates[$dataList[0]->mongo_id]->response_time;
				// $temp_speed = ( 3000 - $site_info['response_speed'] ) / 3000 * 100;
				$site_info['response_speed'] = round( $site_info['response_speed'] / 1000 * 100 ) / 100;
				$site_info['response_speed'] .= " sec";
			}
		}
	...
	function getCheckSiteListByUserId( $user_id ){
		...
		if( is_array( $datalist ) && count( $datalist ) > 0 ){
			$mongoAPI = new MongoAPI();
			$lastDayCheckStates = $mongoAPI->getMultiCheckLastDayStatus( $mongo_id_list_after_one_day, $mongo_id_list_less_one_day );	
//			Log::info(print_r($lastDayCheckStates, true));
