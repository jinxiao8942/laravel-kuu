...
class MongoAPI extends Eloquent{
	...
	public function saveSiteInfoToMongo( $check_type, $check_name, $check_url, $match=NULL ){
		$curl = curl_init();

		// if type=dns, add default dns server 8.8.8.8 (google dns) to url
		if ($check_type == "dns")
			$check_url = "8.8.8.8/" . $check_url . ( $match ? "/{$match}" : "");

		if ( ( ($check_type == "http") || ($check_type == "https") ) && $match )
			$check_url .= "::find::{$match}";

		$data = "type={$check_type}&name={$check_name}&url={$check_url}&alert_threshold={$this->alert_threshold}&response_threshold={$this->response_threshold}&interval={$this->interval}";
	...
	public function updateCheckInMongo( $mongo_id, $type, $name, $url, $is_paused, $match=NULL ){

		// if type=dns, add default dns server 8.8.8.8 (google dns) to url
		if ($type == "dns")
			$url = "8.8.8.8/" . $url . ( $match ? "/{$match}" : "");

		if ( ( ($type == "http") || ($type == "https") ) && $match )
			$url .= "::find::{$match}";
		...
		curl_setopt( $curl, CURLOPT_POST, 1 );
		curl_setopt( $curl, CURLOPT_POSTFIELDS, "_id={$mongo_id}&type={$type}&name={$name}&url={$url}&alert_threshold={$this->alert_threshold}&response_threshold={$this->response_threshold}&interval={$this->interval}&is_paused={$is_paused}" );
		...
		if(  is_array( $mongo_id_list_after_one_day ) && count(  $mongo_id_list_after_one_day ) > 0 ){
			...
			foreach( $resp_result as $res ){
				if( $res->Result == "OK" && is_array($res->Records) && count($res->Records) > 0 ){				
					...
					if(isset($res->CheckInfo) && isset($res->CheckInfo->is_up) && $res->CheckInfo->is_up == 0)
						$datalist[$res->Records[0]->check_id]->availability = 0;
